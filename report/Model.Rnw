...
6. Once we identified the best model we use the \textbf{test_set} to calculate the test MSE, which will eventually help us compare the performances of all the models.
...
3. Run \texttt{pcr()} or \texttt{plsr()} depending on which model you want, and use arguments \texttt{Grad_Rate ~ ., data=train_set, validation = CV and scale = TRUE}.
...
\begin{figure}
\centering
\graphicspath{../images/Ridge_plot.png}
\includegraphics{Ridge_plot}
\caption{MSE Against lambda values for Ridge regression}
\end{figure}

\begin{figure}
\centering
\graphicspath{../images/Lasso_plot.png}
\includegraphics{Lasso_plot}
...
\begin{figure}
\centering
\graphicspath{../images/pcr_validationplot.png}
\includegraphics{pcr_validation}
\caption{Validation Plot for PC Regression}
\end{figure}  

\begin{figure}
\centering
\graphicspath{../images/Plsr_validation.png}
\includegraphics{Ridge_plot}
...
From the table of regression coefficients, $%#$%#$%#$%#$%#
